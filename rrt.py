...
while True:
    ...
    if pathNotFound:
        ...
        node.edges.append(closestNode.id)
        closestNode.edges.append(node.id)
        ...
        if inGoal(node):
            ...
            while queue:
                ...
                if type(currentNode) == Node:
                    for adjNodeId in currentNode.edges:
                        if visited[adjNodeId] == False:
                            queue.append(nodes[adjNodeId])
                            parent[adjNodeId] = currentNode.id
                            visited[adjNodeId] = True

            currentParent = node.id
            while parent[currentParent] != None:
                if type(nodes[currentParent]) == Node and type(nodes[parent[currentParent]]) == Node:
                    canvas.create_line(nodes[currentParent].x, nodes[currentParent].y, nodes[parent[currentParent]].x, nodes[parent[currentParent]].y, fill = "darkgoldenrod1", width = 3)
                
                currentParent = parent[nodes[currentParent].id]
